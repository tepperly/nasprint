...
CONTEST_START=Time.utc(2025,10,4,16, 00)
CONTEST_END=Time.utc(2025,10,5,22,00)
...
class Cabrillo
  ...
  def checkTime(qso)
    if qso.datetime < CONTEST_START
      print "QSO in #{qso.sentExch.callsign}'s log has a date #{qso.datetime} before contest start\n"
    end
    if qso.datetime > CONTEST_END
      print "QSO in #{qso.sentExch.callsign}'s log has a date #{qso.datetime} after contest end\n"
    end
  end
