...
class ContestDatabase
  ...
  def addLog(contID, callsign, callID, email, powclass, opclass, multID, entID, name, club, locator, numops, isCCE, isMobile, isNew, isSchool, isYL, isYouth, isOneDay, isCountyLine, isSundayOnly)
    @db.query("insert into Log (contestID, callsign, callID, email, powclass, opclass, multiplierID, entityID, name, club, locator, numops, isCCE, isMOBILE, isNEW, isSCHOOL, isYL, isYOUTH, isONEDAY, isCOUNTYLINE, isSUNDAYONLY) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
              [contID.to_i, capOrNull(callsign), callID.to_i, email, powclass, 
                opclass, multID.to_i, numOrNull(entID), name, club, locator, numops,
                @db.boolToDB(isCCE), @db.boolToDB(isMobile), @db.boolToDB(isNew),
                @db.boolToDB(isSchool), @db.boolToDB(isYL), @db.boolToDB(isYouth),
                @db.boolToDB(isOneDay), @db.boolToDB(isCountyLine), @db.boolToDB(isSundayOnly)
              ]) { }
