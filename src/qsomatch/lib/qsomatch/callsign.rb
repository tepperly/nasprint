...
class CallsignTools
  CALLSIGN_ENCODING=Encoding::ASCII

  def initialize
    @exceptions = Set.new
  end

  def addException(val)
    if val.kind_of?(String)
      @exceptions << val.upcase.encode(CALLSIGN_ENCODING)
    elsif val.respond_to?(:each) 
      val.each { |v|
        addException(v)
      }
    else
      raise ArgumentError.new("addException: only takes strings and containers with strings")
    end
  end
  ...
  def callBase(str)
    str = str.upcase.encode(CALLSIGN_ENCODING)
    return str if @exceptions.include?(str)
    str.gsub!(/\s+/,"")
    parts = str.split("/")
    case parts.length
    when 0
