...
def lookupMult(db, id)
  res = db.query("select abbrev from Multiplier where id = ? limit 1;", [id])
  res.each { |row|
    return row[0]
  }
  nil
...
def fillInComment(db, contestID)
  ...
  res.each { |row|
    ...
    if comments.empty?
      ...
      db.query("update QSO set comment=? where id = ? limit 1;",
               [comments.join(", "), row[0]])
    end
  }
